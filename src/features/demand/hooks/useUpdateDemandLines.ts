...
export const useUpdateDemandLines = (
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t('demand.lines.updateSuccess', 'Satırlar güncellendi'));
    },
    onError: (error: Error) => {
      toast.error(error.message ?? t('demand.lines.updateError', 'Satırlar güncellenirken bir hata oluştu'));
    },
  });
};
