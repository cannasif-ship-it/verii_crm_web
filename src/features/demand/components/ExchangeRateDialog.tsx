        ...
        <DialogHeader className="p-6 pb-4 border-b border-zinc-200/50 dark:border-zinc-800/50 bg-zinc-50/30 dark:bg-zinc-900/10">
          <DialogTitle className="flex items-center gap-3 text-xl">
            ...
            <div className="flex flex-col gap-0.5">
              <span className="text-zinc-900 dark:text-zinc-100">{t('demand.exchangeRates.dialog.title', 'Döviz Kurları')}</span>
              <span className="text-xs font-normal text-muted-foreground">{t('demand.exchangeRates.dialog.subtitle', 'Güncel kur değerlerini yönetin')}</span>
            </div>
          </DialogTitle>
          <DialogDescription className="hidden">
            {t('demand.exchangeRates.dialog.description', 'Kur değerlerini görüntüleyin ve gerekirse düzenleyin.')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="p-6">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3 text-muted-foreground">
              ...
              <span className="text-sm font-medium">{t('demand.loading', 'Kurlar yükleniyor...')}</span>
            </div>
          ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent border-0">
                    <TableHead className={cn(styles.tableHead, "pl-6")}>{t('demand.exchangeRates.currency', 'Para Birimi')}</TableHead>
                    <TableHead className={cn(styles.tableHead, "text-right")}>{t('demand.exchangeRates.rate', 'Kur Değeri')}</TableHead>
                    <TableHead className={cn(styles.tableHead, "text-center")}>{t('demand.exchangeRates.status', 'Durum')}</TableHead>
                    <TableHead className={cn(styles.tableHead, "text-center w-[100px]")}>{t('demand.actions', 'İşlemler')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {localRates.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center py-8 text-muted-foreground">
                        {t('demand.exchangeRates.empty', 'Kur bulunamadı')}
                      </TableCell>
                    </TableRow>
                  ) : (
                    localRates.map((rate) => {
                          ...
                          <TableCell className="text-center">
                            {rate.isOfficial ? (
                              <Badge variant="secondary" className="bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800 text-[10px] px-2 h-5">
                                {t('demand.exchangeRates.official', 'Resmi')}
                              </Badge>
                            ) : (
                              <Badge variant="secondary" className="bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800 text-[10px] px-2 h-5">
                                {t('demand.exchangeRates.custom', 'Özel')}
                              </Badge>
                            )}
                          </TableCell>
                          ...
                          <TableCell className="text-center">
                            {editingId === rate.id ? (
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  if (isUsed) {
                                    toast.error(t('demand.exchangeRates.cannotEditUsedCurrency', 'Kullanımda olan kur değiştirilemez'));
                                    return;
                                  }
          ...
          <div className="mt-4 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-lg p-3 text-xs text-blue-600 dark:text-blue-400 flex gap-2 items-start">
             ...
             <p>{t('demand.exchangeRates.dialog.info', 'Burada yapılan değişiklikler sadece bu teklif için geçerlidir ve genel sistem kurlarını etkilemez. Değiştirilen kurlar "Özel" olarak işaretlenir.')}</p>
          </div>
        </div>
        ...
        <DialogFooter className="p-4 border-t border-zinc-200/50 dark:border-zinc-800/50 bg-zinc-50/30 dark:bg-zinc-900/10 gap-2">
          ...
          >
            {isSaving ? (
              ...
              t('demand.saveAndApply', 'Kaydet ve Uygula')
            )}
          </Button>
        </DialogFooter>
      </DialogContent>
