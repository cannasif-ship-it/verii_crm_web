    ...
    <div className={styles.glassCard}>
      <div className="p-6">
        <div className="flex items-center gap-2 mb-6 text-zinc-900 dark:text-white">
          ...
          <h3 className="font-bold text-base">{t('demand.summary.title', 'Özet')}</h3>
        </div>

        {/* Hesaplamalar */}
        <div className="space-y-3">
          <div className={styles.row}>
            <span className={styles.label}>{t('demand.summary.subtotal', 'Ara Toplam')}</span>
            <span className={styles.value}>{formatCurrency(totals.subtotal, currencyCode)}</span>
          </div>
          
          <div className={styles.row}>
            <span className={styles.label}>{t('demand.summary.totalVat', 'Toplam KDV')}</span>
