    ...
    <div className={styles.glassCard}>
      ...
      <div className="p-5 border-b border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-900/20">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-base font-bold text-zinc-900 dark:text-white">
              {t('demand.exchangeRates.title', 'Döviz Kurları')}
            </h3>
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-12 items-end">
          ...
          <div className="md:col-span-3">
            <Label className={styles.label}>{t('demand.exchangeRates.currency', 'Para Birimi')}</Label>
            <Select
          ...
          {/* Kur Değeri */}
          <div className="md:col-span-3">
            <Label className={styles.label}>{t('demand.exchangeRates.rate', 'Kur Değeri')}</Label>
          ...
          {/* Tarih */}
          <div className="md:col-span-3">
            <Label className={styles.label}>{t('demand.exchangeRates.date', 'Kur Tarihi')}</Label>
            <div className="relative">
              <div className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 pointer-events-none">
          ...
          <div className="md:col-span-3 flex items-center justify-between gap-4">
            <div className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-800/50 px-3 h-10 rounded-xl border border-zinc-200 dark:border-zinc-800">
              ...
              <Label htmlFor="isOfficial" className="mb-0 text-[10px] font-medium cursor-pointer">
                {t('demand.exchangeRates.isOfficial', 'Resmi')}
              </Label>
            </div>
      ...
      {exchangeRates.length > 0 ? (
          ...
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent border-0">
                ...
                <TableHead className={cn(styles.tableHead, "text-right")}>{t('demand.exchangeRates.rate', 'Kur')}</TableHead>
                <TableHead className={cn(styles.tableHead, "text-center")}>{t('demand.exchangeRates.date', 'Tarih')}</TableHead>
                <TableHead className={cn(styles.tableHead, "text-center")}>{t('demand.exchangeRates.isOfficial', 'Durum')}</TableHead>
                <TableHead className={cn(styles.tableHead, "text-center w-[80px]")}>{t('demand.actions', 'İşlem')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
        ...
        <div className="flex flex-col items-center justify-center py-8 text-center text-zinc-400 dark:text-zinc-600">
          <p className="text-sm">{t('demand.exchangeRates.empty', 'Henüz eklenmiş özel kur bulunmuyor.')}</p>
        </div>
      )}
    </div>
  );
