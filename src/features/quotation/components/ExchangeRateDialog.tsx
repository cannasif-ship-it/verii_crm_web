      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto p-0 gap-0 overflow-hidden bg-white/95 dark:bg-[#0c0516]/95 backdrop-blur-xl border-zinc-200 dark:border-white/10 shadow-2xl">
        ...
        <DialogHeader className="p-6 pb-5 border-b border-zinc-200/50 dark:border-white/5 bg-zinc-50/30 dark:bg-white/5">
          <DialogTitle className="flex items-center gap-3 text-xl">
            ...
            <div className="flex flex-col gap-0.5">
              <span className="text-zinc-900 dark:text-zinc-100">{t('quotation.exchangeRates.dialog.title', 'Döviz Kurları')}</span>
              <span className="text-xs font-normal text-slate-500 dark:text-slate-400">{t('quotation.exchangeRates.dialog.subtitle', 'Güncel kur değerlerini yönetin')}</span>
            </div>
          </DialogTitle>
          <DialogDescription className="hidden">
            {t('quotation.exchangeRates.dialog.description', 'Kur değerlerini görüntüleyin ve gerekirse düzenleyin.')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="p-6">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent border-0 bg-zinc-50/50 dark:bg-white/5">
                    <TableHead className={cn(styles.tableHead, "pl-6 text-zinc-600 dark:text-zinc-400")}>{t('quotation.exchangeRates.currency', 'Para Birimi')}</TableHead>
                    <TableHead className={cn(styles.tableHead, "text-right text-zinc-600 dark:text-zinc-400")}>{t('quotation.exchangeRates.rate', 'Kur Değeri')}</TableHead>
                    <TableHead className={cn(styles.tableHead, "text-center text-zinc-600 dark:text-zinc-400")}>{t('quotation.exchangeRates.status', 'Durum')}</TableHead>
                    <TableHead className={cn(styles.tableHead, "text-center w-[100px] text-zinc-600 dark:text-zinc-400")}>{t('quotation.actions', 'İşlemler')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {localRates.length === 0 ? (
                          ...
                          <TableCell className="text-center">
                            {editingId === rate.id ? (
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  if (isUsed) {
                                    toast.error(t('quotation.exchangeRates.cannotEditUsedCurrency', 'Kullanımda olan kur değiştirilemez'));
                                    return;
                                  }
          ...
          <div className="mt-4 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-xl p-4 text-xs text-blue-600 dark:text-blue-400 flex gap-3 items-start shadow-sm">
             ...
             <p className="leading-relaxed opacity-90">{t('quotation.exchangeRates.dialog.info', 'Burada yapılan değişiklikler sadece bu teklif için geçerlidir ve genel sistem kurlarını etkilemez. Değiştirilen kurlar "Özel" olarak işaretlenir.')}</p>
          </div>
        </div>
