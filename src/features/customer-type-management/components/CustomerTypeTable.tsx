...
interface CustomerTypeTableProps {
  customerTypes: CustomerTypeDto[]; // <-- Veriyi artık dışarıdan alıyoruz
  isLoading: boolean;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl border border-slate-200 dark:border-white/10 overflow-hidden bg-white/50 dark:bg-transparent">
        <Table>
            ...
            <TableBody>
              {paginatedData.map((customerType: CustomerTypeDto, index: number) => (
                <TableRow 
                ...
                >
                  {tableColumns.filter(col => visibleColumns.includes(col.key)).map((col) => (
                      <TableCell key={`${customerType.id}-${col.key}`} className={`${cellStyle} ${col.className || ''}`}>
                          {renderCellContent(customerType, col)}
                      </TableCell>
                  ))}

                  <TableCell className={`${cellStyle} text-right`}>
                    <div className="flex justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-600 hover:text-blue-700 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-500/10" onClick={() => onEdit(customerType)}><Edit2 size={16} /></Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-red-600 hover:text-red-700 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-500/10" onClick={() => handleDeleteClick(customerType)}><Trash2 size={16} /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
