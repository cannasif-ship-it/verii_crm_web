...
export function UserGroupAssignmentsPage(): ReactElement {
  ...
  useEffect(() => {
    setPageTitle(t('userGroupAssignments.title', 'User Group Assignments'));
  ...
  return (
    <div className="w-full space-y-6">
      <Breadcrumb items={[{ label: t('sidebar.accessControl', 'Access Control') }, { label: t('sidebar.userGroupAssignments', 'User Group Assignments'), isActive: true }]} />
      <div className="flex flex-col gap-2 pt-2">
        <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white transition-colors">
          {t('userGroupAssignments.title', 'User Group Assignments')}
        </h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm font-medium transition-colors">
          {t('userGroupAssignments.description', 'Assign permission groups to users')}
        </p>
      ...
      <div className="bg-white/70 dark:bg-[#1a1025]/60 backdrop-blur-xl border border-white/60 dark:border-white/5 shadow-sm rounded-2xl p-5 space-y-4">
        <div>
          <label className="text-sm font-medium mb-2 flex items-center gap-1">
            {t('userGroupAssignments.selectUser', 'Select User')}
          ...
          <Combobox
            ...
            placeholder={t('userGroupAssignments.selectUserPlaceholder', 'Search and select user...')}
            searchPlaceholder={t('common.search', 'Search')}
            emptyText={t('userGroupAssignments.noUsers', 'No users found')}
        ...
        {selectedUserId != null && (
          <div>
            <label className="text-sm font-medium mb-2 flex items-center gap-1">
              {t('userGroupAssignments.assignedGroups', 'Assigned Permission Groups')}
              <FieldHelpTooltip text={t('help.userAssignment.groups')} />
            </label>
            {userGroupsLoading ? (
              <div className="py-8 text-center text-slate-500">{t('common.loading', 'Loading...')}</div>
            ) : (
              <>
                ...
                {hasChanges && (
                  <div className="mt-4 flex justify-end items-center gap-1">
                    ...
                    <Button onClick={handleSave} disabled={setUserGroups.isPending}>
                      {setUserGroups.isPending ? t('common.saving', 'Saving...') : t('common.save', 'Save')}
                    </Button>
                  </div>
                )}
              </>
        ...
        {!selectedUserId && !usersLoading && (
          <div className="py-12 text-center text-slate-500 dark:text-slate-400">
            {t('userGroupAssignments.selectUserHint', 'Select a user to manage their permission group assignments')}
          </div>
        )}
      </div>
    </div>
