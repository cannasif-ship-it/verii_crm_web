...
export function PowerbiReportsListPage(): ReactElement {
  ...
  useEffect(() => {
    setPageTitle(t('powerbiViewer.listTitle', 'PowerBI Raporları'));
    ...
    <div className="w-full space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">
          {t('powerbiViewer.listTitle', 'PowerBI Raporları')}
        </h1>
        <p className="text-muted-foreground mt-1">
          {t('powerbiViewer.listDescription', 'Raporlara tıklayarak görüntüleyebilirsiniz')}
        </p>
      </div>
      <div className="rounded-md border">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>{t('powerbiViewer.name', 'Ad')}</TableHead>
              <TableHead>{t('powerbiViewer.description', 'Açıklama')}</TableHead>
              <TableHead>{t('powerbiViewer.isActive', 'Durum')}</TableHead>
              <TableHead className="text-right w-[120px]">{t('common.actions', 'İşlemler')}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {items.length === 0 ? (
                  ...
                  <TableCell className="text-right">
                    <Button variant="outline" size="sm" asChild>
                      <Link to={`/powerbi/reports/${row.id}`}>
                        ...
                        {t('powerbiViewer.view', 'Görüntüle')}
                      </Link>
                    </Button>
                  </TableCell>
                </TableRow>
